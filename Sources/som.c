...
//parametros internos da rede
    ...
    t_float   x_trainingmode; //modo de treinamento
    t_float   x_proba; // probabilidade de selecionar um neurônio vencedor com a menor distância euclidiana
...
//verifica se a lista recebida tem tamnaho 2 e atribui os valores para cada variável
    ...
    if(x->x_netsize[0] <= MAXNEURON && x->x_netsize[1] <= MAXVECTOR){
        ...
        error("the maximum size of neurons and vectors has been exceeded");
    }
...
/// --------------------- ordena o vetor x->win e encontra o indice da menor distância no vetor x->dist ----------------//
    int win_index;
...
//------------------------------- encontra o indice da menor distância correspondente ao vetor x->dist ------------------------------//

    for (i = 0; i < x->x_neurons; i++){
        if (gsl_vector_get(x->dist, i) == gsl_vector_get(x->win, 0)) { 
            win_index = i;
...
// encontra o proba_index correspondente ao vetor x->dist
     int proba_index;
    for (i = 0; i < x->x_neurons; i++){
        if (gsl_vector_get(x->dist, i) == gsl_vector_get(x->win, indice)) { 
        proba_index = i;
        }
    }
...
// função de vizinhança
    ...
    for (i = 0; i < x->x_topo; i++){
        gsl_vector_set(x->vizi, i, gsl_vector_get(x->win, i+1)); //armazena os neurônios vizinhos no vetor x->vizi
...
//------------------------ propabilidade do neurônio vencedor ter a menor distância euclidiana () --------------------//
static void som_proba(t_som *x, t_floatarg f)
{
  t_float proba;
  // int i;
  ...
  post("probabilidade: %0.3f", x->x_proba);
...
static void *som_new(t_symbol *s, int argc, t_atom *argv){ //argc é a quantidade de elementos da lista e argv é um ponteiro para uma lista
    ...
    //  t_float* x_netsize = (t_float*)malloc(NET * sizeof(t_float));
...
void som_setup(void) {
    ...
    class_addmethod(som_class, (t_method)som_proba, gensym("proba"), A_FLOAT, 0);
}